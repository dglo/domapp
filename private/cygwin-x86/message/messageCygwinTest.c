...
int messageCygwinTest() {
    ...
    if(threadCounter != 2) {
  	errorMsg = "messageCygwinTest: basic pthread test failure";
    	return ERROR;
    }
    ...
    if(threadCounter != 2) {
  	errorMsg = 
	    "messageCygwinTest: basic pthread condition variable test failure";
   	return ERROR;
    }
    ...
    if(oneBuffer == 0) {
	errorMsg="messageCygwinTest: unable to allocate message buffer";
	return ERROR;
    }
	...
	for(j=0;j<=MAX_SUBTYPE;j++) {
		...
		if(Message_getType(oneBuffer) != type) {
		    errorMsg=
			"messageCygwinTest: type set/read error";
		    return ERROR;
		}
		if(Message_getSubtype(oneBuffer) != subType) {
		    errorMsg=
			"messageCygwinTest: subtype set/read error";
		    return ERROR;
		}
		if(Message_getStatus(oneBuffer) != status) {
		    errorMsg=
			"messageCygwinTest: status set/read error";
		    return ERROR;
		}
	    }
	}
	...
	Message_setDataLen(oneBuffer,dataLen);
 	if(Message_dataLen(oneBuffer) != dataLen) {
	    errorMsg="messageCygwinTest: data length set/read error";
	    return ERROR;
	}
    }
	...
	errorMsg="messageCygwinTest: MAXDATA_VALUE data length set/read error";
	return ERROR;
    }
    ...
    if(queue < 0) {
	errorMsg="messageCygwinTest: cannot create message queue";
	return ERROR;
    }
    ...
    if (receive != -1) {
	errorMsg=
	"messageCygwinTest: incorrect return from non blocking call on empty queue";
	return ERROR;
    }
	...
	errorMsg="messageCygwinTest: incorrect send/receive message pair";
  	return ERROR;
    }
    ...
    if(msgPutCounter != msgGetCounter) {
  	errorMsg = "messageCygwinTest: threaded msg queue test failed";
    	return ERROR;
    }

    /* done--no detected errors */
    errorMsg="messageCygwinTest: success";
    return 0;

}

char *messageCygwinTest_status() {
    return errorMsg;
}
