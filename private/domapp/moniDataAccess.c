...
void moniInsertSelfLCModeChangeMessage(unsigned long long time, UBYTE mode) {
  ...
  mr.time = time;
  mr.data[0] = DOM_SLOW_CONTROL;
  mr.data[1] = DSC_SET_SELF_LC_MODE;
  mr.data[2] = mode;
  moniInsertRec(&mr);
}

void moniInsertSelfLCWindowChangeMessage(unsigned long long time,
					 ULONG self_ns) {
  struct moniRec mr;
  mr.dataLen = 2+sizeof(ULONG);
  mr.moniEvtType = MONI_TYPE_CONF_STATE_CHG_MSG;
  mr.time = time;
  mr.data[0] = DOM_SLOW_CONTROL;
  mr.data[1] = DSC_SET_SELF_LC_WINDOW;
  formatLong(self_ns,  &(mr.data[2]));
  moniInsertRec(&mr);
}
